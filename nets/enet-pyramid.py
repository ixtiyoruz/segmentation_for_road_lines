...
class EnetNet():
    ...
    def decoder_bottleneck(self, x, output_depth, scope, proj_ratio=4,
        ...
        return output
    def SpatialContextBlock(self, input_tensor, scope,
                level,
                conv_filters=512,
                pooling_type='avg',
                use_batchnorm=True,
        ...
        Pooling2D = tf.nn.max_pool if pooling_type == 'max' else tf.nn.avg_pool
        ...
        input_shape =input_tensor.x.get_shape().as_list()
        ...
        x = tf.nn.avg_pool(input_tensor, ksize=pool_size, strides=pool_size,padding="SAME")
        W_proj = self.get_variable_weight_decay(scope + "/W_proj",
                        shape=[1, 1, input_shape[-1], conv_filters], # ([filter_height, filter_width, in_depth, out_depth])
                        initializer=tf.contrib.layers.xavier_initializer(),
                        loss_category="encoder_wd_losses")
        x = tf.nn.conv2d(x, W_proj, strides=[1, 1, 1, 1],
                    padding="VALID", nam) # NOTE! no bias terms
        x= tf.image.resize_bilinear(input_tensor, [spatial_size[0], spatial_size[1]], name =scope+"/" + upsampling_name)
