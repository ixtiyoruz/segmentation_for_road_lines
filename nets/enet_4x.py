...
class EnetNet():
    ...
    def initial_block(self, x, scope):
        ...
        W_conv = self.get_variable_weight_decay(scope + "/W",
                    shape=[3, 3, 1, 13], # ([filter_height, filter_width, in_depth, out_depth])
    ...
    def add_logits(self, imgs_ph):
        ...
        AT_GEN1 = self.encoder_bottleneck_dilated(x=network, output_depth=128,
        ...
        # layer 3:
        network = self.encoder_bottleneck_regular(x=AT_GEN1, output_depth=128,
        ...
        network = self.encoder_bottleneck_regular(x=AT_GEN3, output_depth=128,
                    drop_prob=self.late_drop_prob_ph, scope="bottleneck_5_1")
        print(network.get_shape().as_list())
    
        network = self.encoder_bottleneck_dilated(x=network, output_depth=128,
                    drop_prob=self.late_drop_prob_ph, scope="bottleneck_5_2",
                    dilation_rate=2)
        print(network.get_shape().as_list())
    
        network = self.encoder_bottleneck_asymmetric(x=network, output_depth=128,
                    drop_prob=self.late_drop_prob_ph, scope="bottleneck_5_3")
        print(network.get_shape().as_list())
    
        network = self.encoder_bottleneck_dilated(x=network, output_depth=128,
                    drop_prob=self.late_drop_prob_ph, scope="bottleneck_5_4",
                    dilation_rate=4)
        print(network.get_shape().as_list())
    
        network = self.encoder_bottleneck_regular(x=network, output_depth=128,
                    drop_prob=self.late_drop_prob_ph, scope="bottleneck_5_5")
        print(network.get_shape().as_list())
    
        network = self.encoder_bottleneck_dilated(x=network, output_depth=128,
                    drop_prob=self.late_drop_prob_ph, scope="bottleneck_5_6",
                    dilation_rate=8)
        print(network.get_shape().as_list())
    
        network = self.encoder_bottleneck_asymmetric(x=network, output_depth=128,
                    drop_prob=self.late_drop_prob_ph, scope="bottleneck_5_7")
        print(network.get_shape().as_list())
    
        AT_GEN4 = self.encoder_bottleneck_dilated(x=network, output_depth=128,
                    drop_prob=self.late_drop_prob_ph, scope="bottleneck_5_8",
                    dilation_rate=16)
